...
public class Utility {
	...
	public static String getFileFormat(File obj, String fileName)
			throws Exception {
		if (obj.exists()) {
			String extension = "";
	...
	public static double currencyConverter(String currency, int amount) {
		...
		if (Currency.SGP.name().equals(currency)) {
			...
			System.out.println("SGD " + result + " amount " + amount
					+ " Currency.SGD.value " + Currency.SGP.value);
			return result;
		}
		if (Currency.USD.name().equals(currency)) {
			double result = amount / Currency.USD.value;
			return result;
		}
		return 0;

	...
	public Map<String, List<IncomeData>> loadData(File obj) {
		...
		try {
			...
			while ((line = br.readLine()) != null) {
				...
				String city_country=country[1]!=null && !country[1].isEmpty()? country[1]: country[0];

				double convertedAverage=Utility.currencyConverter(country[3], Integer.parseInt(country[4]));
				convertedAverage= Double.valueOf(dtime.format(convertedAverage));

				if(map.containsKey(city_country)){
					List ll=map.get(city_country);
					ll.add(new IncomeData(country[0],country[1],country[2],country[3],convertedAverage));}
				else{
					List<IncomeData> list=new ArrayList<IncomeData>();
					list.add(new IncomeData(country[0],country[1],country[2],country[3],convertedAverage));
					map.put(city_country, list);
				}
	...
	public File generateFile(Map<String, List<IncomeData>> data,String outputFile){
		...
		try
		{
			...
			for (String key : data.keySet()) {
				List<IncomeData> value = data.get(key);
				GenderValue v=getIncomeByGender(value);
